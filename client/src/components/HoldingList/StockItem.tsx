...
const StockItem: React.FC<StockItemProps> = ({ companyData, stockData }) => {
  ...
  const postMutation: ReturnType<typeof usePostStar> = usePostStar();
  const deleteMutation: ReturnType<typeof useDeleteStar> = useDeleteStar();
      ...
      >
        <LogoContainer>
          ...
          <FavoriteStar
            ...
            opacity={isHovering && !isFavorited ? 1 : 0}
          />
          <FavoriteStarFilled
            onClick={toggleFavorite}
            opacity={isHovering && isFavorited ? 1 : 0}
